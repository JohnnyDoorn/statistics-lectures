...
```{r, echo=TRUE}
expCondition = sample(c("exp", "control"), dim(data)[1], replace = TRUE)
```
...
datatable(data[,c('expCondition', 'IQ.you')], options = list(searching = FALSE, scrollY = 415, paging = F, info = F))
```
...
IQ.you.control = subset(data, gender == "control", select = IQ.you)$IQ.you
