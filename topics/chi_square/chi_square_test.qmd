...
results <- data.frame(Type = rep(c("Even", "Odd"), each = ncol(gResults)),
                      Association = c(c(gResults[["4"]], gResults[["6"]]), c(gResults[["3"]], gResults[["5"]])),
                      NumberTested= rep(c(4,6,3,5), each = ncol(gResults) ))
...
$OR = \frac{\text{feminine}}{\text{masculine}} = \frac{`r odds[1,1] / odds[1,2]`}{`r odds[2,1] / odds[2,2]`} = `r (odds[1,1] / odds[1,2]) / (odds[2,1] / odds[2,2])`$
